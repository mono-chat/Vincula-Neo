...
on 1:sockread:smx.msn:{
  ...
  elseif ($$1 == IRCVERS) { sockwrite -tn smx.c $1- }
  elseif ($$1 == NICK) { sockwrite -tn smx.c NICK Sky | sockclose smx.msn | window -c @web }
...
on *:sockread:translator:{
  ...
  elseif ($1 == JOIN) { 
    if ($sock(smx.sid $+ $hget(channels,$2-)).status == active) {
      sockwrite -tn smx.sid $+ $hget(channels,$2-) JOIN $2-
    }
    else {
...
on 1:sockread:smx.c:{
  ...
  elseif ($2 == 702) { echo -s 4Room Not Found. }
...
on 1:sockread:smx.sid??:{ 
  ...
  if ($1-4 == AUTH GateKeeper S :OK) {
    ...
    sockwrite -tn $sockname  USER Sky "Sky" "mx.com" :SeasonsMX-X
  ...
  elseif ($2 == 433) { sockclose $sockname }
  elseif ($2 == 913) { sockclose $sockname }
  ...
  elseif ($2 == PRIVMSG) {
    if (VERSION isin $4) { echo -s 14[15VERSION14]1:4 $remove($gettok($1,1,33),:) | ctcpreply $remove($gettok($1,1,33),:) VERSION Seasons MX By Sky | halt }
...
on 1:sockread:smx.chan??:{
  ...
  elseif ($$1 == IRCVERS) { sockwrite -tn smx.sid $+ $right($sockname,2) $1- }
  else halt
  ; smx.c is the connecting socket ;-;
}
